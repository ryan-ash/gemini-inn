...
public class InkNarratorService : MonoBehaviour {
    ...
    public string getNextStoryLine() {
        if (story.canContinue) {
            ...
            foreach (var tag in story.currentTags) {
                if (actionsDictionary.ContainsKey(tag)) {
                    Debug.Log("Found tag with action assigned: " + tag);
                    actionsDictionary[tag]();
                }
            }
            return text;
        }
    ...
    public void addObservableToTag(string tag, UnityAction observable) {
        actionsDictionary.Add(tag, observable);
    }
}
